...
# dhcp
dhcp="$(cat /etc/resolv.conf |grep nameserver| sed -e 's/nameserver //g')"

enabled_dns_address=(google level3 opendns norton dhcp)
########################


selected_dns="Unknown"

if [[ $(networksetup -getdnsservers "Wi-Fi") =~ ^There ]]
  then
    IFS=', ' read -r -a current_dns_address <<< "$(cat /etc/resolv.conf |grep nameserver| sed -e 's/nameserver //g' | xargs)"
  else
